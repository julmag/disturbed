...
class ModelSUPERTREX:
    ...
    def noise_adder(arr, noise_variance):
      """ Returns inputed array with noise added on each weight.
            ...
            noise_variance  =  the added noise is a Gaussian Distribution with mu = 0 and sigma=noise_variance. Input as float.
      """
      return arr +  np.random.normal(0, noise_variance,[arr.shape[0],arr.shape[1]])

    @staticmethod
    def noise_adder_percentage(arr, noise_percentage):
      """ Returns inputed array with added noise on each weight. Gaussian Distribution Noise added.
          Mu=0, Signma based on the noise_percentage of avg weights for each output added to each weights of corresponding output.
            arr               = weight array which needs the added noise
            noise_percentage  = How much of the average signal should be added as noice. Determince the variance of noise. Input Range 0.-1..
      """
      noise_variance = np.array(np.mean(arr, axis=1) * noise_percentage)
      noise = np.zeros((arr.shape[0], arr.shape[1]))

      for i in range(noise_variance.shape[0]):
        noise = arr[i,:] +  np.random.normal(0, noise_variance[i],[arr.shape[0],arr.shape[1]])
